...
Each modifier adds a distinct visual overlay to alloy items (ingots, nuggets, rods, plates, tools, armor). These overlays are rendered as separate texture layers using the `minecraft:composite` + `minecraft:condition` item model system, driven by `CustomModelDataComponent.flags()`. Modifier presence is binary — the amount of modifier does not change the visual strength.

| Modifier      | Visual Effect                                                    |
|---------------|------------------------------------------------------------------|
| Coal          | Black and dark gray speckled dots                                |
| Bone Meal     | Bright white edge coloring, fading inward                        |
| Slime Ball    | Green diagonal stripes                                           |
| Clay Ball     | Brown diagonal stripes (perpendicular to Slime)                  |
| Lapis Lazuli  | Dark blue edge coloring, fading inward                           |
| Sugar         | White and bright green speckled dots                             |
| Blaze Powder  | Orange and bright red speckled dots                              |
| Glowstone Dust| Bright golden speckled dots with pulsing brightness (animated)          |
| Redstone      | Bright red speckled dots with pulsing brightness (animated)             |
| Ender Pearl   | Teal-green/purple static swirl pattern (animated color gradient sweep) |
| Meat          | Dense grainy blood-red pattern. Plays slime squish sound on tool hit, armor damage, and item drop. |

Overlay textures are generated by `generate_overlays.py` (run from project root). Each overlay is a 16x16 RGBA PNG with colors baked in (not tinted at runtime). Animated overlays (Glowstone, Redstone, Ender Pearl) use multi-frame PNGs with `.mcmeta` animation metadata.
